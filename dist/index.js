...
run(process.argv[2])
    ...
    .catch((e) => {
    (0, helpers_1.writeError)(e instanceof Error ? e.message : `${e}`);
    process.exit(-1);
});
async function run(command) {
    switch (command) {
        case 'snapshot-create': {
            ...
            if (!snapshotName) {
                snapshotName = (0, helpers_1.requestText)('Enter a name for the snapshot: ');
            }
...
async function createPgClient() {
    const client = new pg_1.Client({ ...databaseConfig, database: 'postgres' });
    (0, helpers_1.writeText)('Connecting to postgres');
...
async function cleanSnapshots() {
    ...
    if (databases.length === 0) {
        (0, helpers_1.writeText)('There are no snapshot databases to remove.');
        return;
    }
    (0, helpers_1.writeWarning)(`This will drop the following snapshot databases: \n${databases.map((db) => `  - ${db}`).join('\n')}`);
    const confirmationText = `Yes I'm sure`;
    const confirmation = (0, helpers_1.requestText)(`Enter "${confirmationText}" to confirm this action: `);
    if (confirmation !== confirmationText)
        throw new Error('Aborted by user');
    for (const db of databases)
        await dropDatabase(client, db);
}
async function createSnapshot(snapshotName) {
    ...
    if (existingDatabases.some((db) => db === snapshotDbName)) {
        (0, helpers_1.writeWarning)(`Snapshot db with the name ${snapshotDbName} already exists.`);
        if ((0, helpers_1.yeahNah)('Would you like to override this snapshot database?')) {
    ...
    (0, helpers_1.writeText)(`Creating snapshot database ${snapshotDbName}`);
    await client.query(`
...
async function restoreSnapshot(snapshotName) {
    ...
    (0, helpers_1.writeWarning)('This will drop the main database and override it with the specified snapshot.');
    if (!(0, helpers_1.yeahNah)('Are you sure you want to continue?'))
        throw new Error('Aborted by user');
    await closeOtherConnections(client, databaseConfig.database);
    await closeOtherConnections(client, snapshotDatabaseName);
    await dropDatabase(client, databaseConfig.database);
    (0, helpers_1.writeText)(`Restoring snapshot from ${snapshotDatabaseName}`);
    await client.query(`CREATE DATABASE ${databaseConfig.database} WITH TEMPLATE = ${snapshotDatabaseName}`);
    if ((0, helpers_1.yeahNah)('Would you like to remove the snapshot?')) {
        await dropDatabase(client, snapshotDatabaseName);
    }
}
async function getSnapshotDatabaseName(client, snapshotName) {
    const databases = await getDatabases(client, databaseConfig.database);
    if (snapshotName) {
        ...
        (0, helpers_1.writeWarning)(`Couldn't find snapshot with the name "${snapshotName}"`);
    }
    (0, helpers_1.writeText)(`Available snapshots: \n${databases.map((db, i) => `  ${i + 1}: ${db.substring(databaseConfig.database.length + 1)}`).join('\n')}`);
    const snapshotNumber = Number((0, helpers_1.requestText)('Enter the number of the snapshot to restore: '));
    if (isNaN(snapshotNumber) || !databases[snapshotNumber - 1])
...
async function dropDatabase(client, databaseName) {
    (0, helpers_1.writeText)(`Dropping snapshot database ${databaseName}`);
    await client.query(`DROP DATABASE ${databaseName}`);
}
async function getDatabases(client, baseName) {
...
async function closeOtherConnections(client, dbName) {
    (0, helpers_1.writeText)(`Closing active connections to ${dbName}`);
    const query = `
...
function generateMigration(name) {
    (0, helpers_1.writeText)(`Generating migration with name: ${name}`);
    (0, helpers_1.runChildProc)('typeorm-ts-node-commonjs', [
        `migration:generate`,
        '--dataSource',
...
function createMigration(name) {
    (0, helpers_1.writeText)(`Creating migration with name: ${name}`);
    (0, helpers_1.runChildProc)('typeorm-ts-node-commonjs', [`migration:create`, '--pretty', `${process.env.TYPEORM_TOOLKIT_MIGRATION_ROOT_DIR}/${name}`]);
}
function checkMigration() {
    (0, helpers_1.writeText)(`Checking if migration is needed`);
    (0, helpers_1.runChildProc)('typeorm-ts-node-commonjs', [
        `migration:generate`,
        '--dryrun',
        '--dataSource',
...
function revertMigration() {
    (0, helpers_1.writeText)(`Reverting the latest migration`);
    (0, helpers_1.runChildProc)('typeorm-ts-node-commonjs', [`migration:revert`, '--dataSource', process.env.TYPEORM_TOOLKIT_MIGRATION_DATASOURCE_CONFIG]);
}
